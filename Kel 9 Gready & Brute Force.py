# Al Zahra Mayluna (075)
# Ayu Ghaniyatur R (178)
# Andreas Setiandi Simorangkir (200)
...
# Definisi graf
graph = {
    'Atambua': {'Labuan Bajo': 2, 'Bea': 4},     
    'Labuan Bajo': {'Atambua': 2, 'Maumere': 4, 'Tambolaka': 1}, 
    'Bea': {'Atambua': 4, 'Waingapu': 3},      
    'Maumere': {'Labuan Bajo': 4, 'Ruteng': 2},      
    'Tambolaka': {'Labuan Bajo': 1, 'Lewoleba': 4},      
    'Waingapu': {'Bea': 3, 'Lewoleba': 2},     
    'Ruteng': {'Maumere': 2, 'Kalabahi': 3},     
    'Lewoleba': {'Tambolaka': 4, 'Waingapu': 2, 'Kalabahi': 5},  
    'Kalabahi': {'Ruteng': 3, 'Lewoleba': 5} 
}
...
# Mencari jalur terpendek dari Atambua ke Kalabahi menggunakan algoritma greedy
...
nx.draw(G, pos, with_labels=True, node_color='skyblue', node_size=6000, font_size=10, font_weight='bold')
...
def draw_graph(graph, shortest_path, all_paths):
    ...
    for path, _ in all_paths:
        path_edges = [(path[i], path[i+1]) for i in range(len(path)-1)]
        nx.draw_networkx_edges(G, pos, edgelist=path_edges, width=1, style='dashed')
